...
/* Lattice Perturbation Theory Example */
double Z0 (unsigned dim, const double *x, void *params)
{

  if (dim != 4) {fprintf(stderr, "dimension must be 4.\n");exit(EXIT_FAILURE);}
  ...
  double pre_factor = 1 / pow((2.0 * K_PI),4.0);

  double dg = 4.0 * sumkhalf+eps;
  double dw = sumk + pow((2.0 * rw * sumkhalf),2.);
